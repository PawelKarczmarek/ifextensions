...
struct fuzzy_node_fcm2
{
	...
	fuzzy_node_fcm2 ** clusters;
};

class fuzzy_c_tree_2_nodes
{
public:
	fuzzy_c_tree_2_nodes(double r, double c, double ** tab);
	~fuzzy_c_tree_2_nodes();
	void remove_tree();
	void build_tree();
	void remove_data();
	double traverse(double record[NMAX]);
	double traverse2(double record[NMAX]);

private:
	void remove_tree(fuzzy_node_fcm2 *leaf);
	void insert(double lower_bounds[NMAX], double upper_bounds[NMAX], fuzzy_node_fcm2 *leaf, int depth = 0);
	...
	fuzzy_node_fcm2 *root;
	int rows;
	int columns;
	double **table;
	double traverse(double record[NMAX], int depth, fuzzy_node_fcm2 *root);
	double traverse2(double record[NMAX], int depth, fuzzy_node_fcm2 *root);
...
fuzzy_c_tree_2_nodes::fuzzy_c_tree_2_nodes(double r, double c, double ** tab)
{
	root = new fuzzy_node_fcm2;
	rows = r;
	columns = c;
...
fuzzy_c_tree_2_nodes::~fuzzy_c_tree_2_nodes()
{
	remove_tree();
}

void fuzzy_c_tree_2_nodes::remove_tree()
{
	remove_tree(root);
}

void fuzzy_c_tree_2_nodes::remove_tree(fuzzy_node_fcm2 *leaf)
{
	if (leaf != NULL)
	{
...
void fuzzy_c_tree_2_nodes::remove_data()
{
	for (int i = 0; i < rows; i++)
		delete[] table[i];
	delete[]table;
...
void fuzzy_c_tree_2_nodes::build_tree()
{
	double lower_bounds[NMAX];
	double upper_bounds[NMAX];
...
void fuzzy_c_tree_2_nodes::insert(double lower_bounds[NMAX], double upper_bounds[NMAX], fuzzy_node_fcm2 *root, int depth)
{
	if (depth > MAX_DEPTH)
	{
	...
	else
	{
		...
		root->clusters = new fuzzy_node_fcm2*[kmax];
		for (int i = 0; i<root->number_of_clusters; i++)
		{
			for (int j = 0; j < columns; j++)
			{
			...
			root->clusters[i] = new fuzzy_node_fcm2;
...
int fuzzy_c_tree_2_nodes::random_attribute()
{
	return rand() % columns;
}

double fuzzy_c_tree_2_nodes::find_attribute_min(int attribute_id)
{
...
double fuzzy_c_tree_2_nodes::find_attribute_max(int attribute_id)
{
...
int fuzzy_c_tree_2_nodes::how_many_elements(double lower_bounds[NMAX], double upper_bounds[NMAX])
{
	int count = 0;
	for (int i = 0; i < rows; i++)
	{
...
double fuzzy_c_tree_2_nodes::traverse(double rekord[NMAX])
{
	return traverse(rekord, 0, root);
}

double fuzzy_c_tree_2_nodes::traverse(double record[NMAX], int depth, fuzzy_node_fcm2 *root)
{
...
double fuzzy_c_tree_2_nodes::traverse2(double record[NMAX])
{
	return traverse2(record, 0, root);
}

double fuzzy_c_tree_2_nodes::traverse2(double record[NMAX], int depth, fuzzy_node_fcm2 *root)
{
	if (root->number_of_clusters>0)
	{
		int attr_id = root->attribute_id;
		...
		if (found)
		{
			for (int i = 0; i < root->number_of_clusters; i++)
			{
				...
				{
					double membership = 0;
					double center = (root->cluster_limits[i] + root->cluster_limits[i + 1]) / 2;
					membership = 1 - abs(record[root->attribute_id] - center) / abs(root->cluster_limits[i + 1] - center);
					return root->average_memberships_to_clusters[i]*membership + traverse2(record, depth, root->clusters[i]);
				}
			}
		}
	}
...
double fuzzy_c_tree_2_nodes::fcm(int K, int attribute_id, double lower_bounds[NMAX], double upper_bounds[NMAX], double **centers, double **limits, double **averages)
{
	...
	int *t_membership_place2 = new int[n];
	...
	for (int j = 0; j < K; j++)
	{
		...
		for (int i = 0; i < n; i++)
		{
			double expression_under_sum = 0;
			if (t_membership[i] == j)
			{
				expression_under_sum += Uk1[i][j];
				int k = t_membership_place2[i];
				counter++;
				sum += expression_under_sum;
			}
		}
		if (counter > 1)
			averages[K][j] = sum/counter;
		else
			averages[K][j] = 0;
	}

	delete[]t_membership;
	delete[]t_membership_place2;
	for (int i = 0; i < n; i++)
		delete[]Uk[i];
	delete[]Uk;
	for (int i = 0; i < n; i++)
